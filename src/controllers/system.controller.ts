...
export const getGeneralReports = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 👇 Query de Saídas/Separações (Já tinha o op_code)
    const separacoesRes = await pool.query(`SELECT s.created_at as data, CASE WHEN s.type='manual' THEN 'Saída - Manual' ELSE 'Saída - Separação' END as tipo, s.destination as destino_setor, cs.op_code, p.name as produto, p.sku, p.unit as unidade, si.quantity as quantidade, COALESCE(CAST(NULLIF(CAST(p.unit_price AS TEXT), '') AS NUMERIC), 0) as preco_unitario FROM separation_items si JOIN separations s ON si.separation_id = s.id JOIN products p ON si.product_id = p.id LEFT JOIN client_services cs ON s.client_service_id = cs.id WHERE s.created_at >= $1 AND s.created_at <= $2 AND s.status = 'concluida' ORDER BY s.created_at DESC`, [start, end]);
    
    // 👇 AQUI FOI ADICIONADO O cs.op_code e o LEFT JOIN client_services para as SOLICITAÇÕES
    const solicitacoesRes = await pool.query(`SELECT r.created_at as data, 'Saída - Solicitação' as tipo, COALESCE(pf.sector, r.sector) as destino_setor, cs.op_code, pf.name as solicitante, COALESCE(p.name, ri.custom_product_name) as produto, p.sku, p.unit as unidade, COALESCE(ri.quantity_delivered, ri.quantity_requested) as quantidade, r.status, COALESCE(CAST(NULLIF(CAST(p.unit_price AS TEXT), '') AS NUMERIC), 0) as preco_unitario FROM request_items ri JOIN requests r ON ri.request_id = r.id LEFT JOIN products p ON ri.product_id = p.id LEFT JOIN profiles pf ON r.requester_id = pf.id LEFT JOIN client_services cs ON r.client_service_id = cs.id WHERE r.created_at >= $1 AND r.created_at <= $2 AND r.status IN ('aprovado', 'entregue') ORDER BY r.created_at DESC`, [start, end]);
