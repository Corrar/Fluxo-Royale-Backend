...
export const updateSeparation = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await createLog(userId, 'UPDATE_SEPARATION_DATA', { separationId: id }, getClientIp(req), client);
    await client.query('COMMIT');
    if ((req as any).io) (req as any).io.emit('separations_update');
    res.json({ success: true });
  } catch (error: any) {
...
export const createReturn = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await createLog(userId, 'CREATE_RETURN', { separationId: id }, getClientIp(req), client);
    await client.query('COMMIT');
    if ((req as any).io) (req as any).io.emit('separations_update');
    res.status(201).json({ success: true });
  } catch (error: any) {
...
export const updateReturnStatus = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await createLog(userId, 'UPDATE_RETURN', { returnId, status }, getClientIp(req), client);
    await client.query('COMMIT');
    if ((req as any).io) (req as any).io.emit('separations_update');
    res.json({ success: true });
  } catch (error: any) {
