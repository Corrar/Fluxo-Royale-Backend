...
export const updateRequestStatus = async (req: Request, res: Response) => {
  ...
  try {
    ...
    await createLog(userId, status === 'entregue' ? 'APPROVE_REQUEST' : status === 'rejeitado' ? 'REJECT_REQUEST' : status === 'devolvido' ? 'RETURN_REQUEST' : 'UPDATE_REQUEST_STATUS', { requestId: id, newStatus: status, reason: rejection_reason }, getClientIp(req), client);
    await client.query('COMMIT');
